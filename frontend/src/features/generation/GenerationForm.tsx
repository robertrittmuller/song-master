...
export function GenerationForm() {
  ...
  const lockedSettingCount = [
    ...
    vocalGender,
    rhymeScheme,
    selectedInstrumentCount ? "instruments" : "",
    albumId ? "album" : ""
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    const normalizedGenre = normalizeOptionalText(genre);
    const normalizedTempo = normalizeOptionalText(tempo);
    const normalizedKey = normalizeOptionalText(key);
    const normalizedMood = normalizeOptionalText(mood);
    const normalizedVocalGender = normalizeOptionalText(vocalGender);
    const normalizedRhymeScheme = normalizeOptionalText(rhymeScheme);
    const normalizedStyle = normalizeOptionalText(style);
    const normalizedInstruments = buildInstrumentValue(selectedInstruments, customInstruments);
      ...
      <Card>
        <div className="generation-hero">
          <div className="generation-hero__main">
            ...
            <div className="generation-field">
              <label className="generation-label" htmlFor="song-prompt">Song Description</label>
              <textarea
                id="song-prompt"
                required
                value={prompt}
                ...
                rows={8}
                placeholder="Describe the story, energy, perspective, references, must-include lines, or production ideas..."
      ...
      >
        <summary className="generation-advanced__summary">
          ...
          <span className="generation-advanced__badge">
            {selectedInstrumentCount + [genre, tempo, key, mood, vocalGender, rhymeScheme].filter(Boolean).length} overrides
          </span>
        </summary>

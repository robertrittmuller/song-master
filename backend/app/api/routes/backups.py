...
def export_backup(
    ...
    """Download a ZIP backup containing the current account's content files and records."""
    buffer = create_backup_zip(db, current_user)
    filename = (
        f"song-master-{current_user.username}-backup-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}.zip"
    )
...
async def restore_backup(
    ...
    try:
        return restore_backup_zip(db, raw_zip, current_user, dry_run=dry_run)
